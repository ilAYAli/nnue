...
Step 1: fresh self-play.
Generate `15-30M` usable fresh Enyo self-play positions from the current
...
Convert self-play PGN to JSONL rows. Filter duplicate FENs, timeout/emergency
moves, mate scores, missing scores, and extreme-only buckets.

Step 3: signed bucket sampling.
Sample score buckets so the data is not mostly neutral positions: `0-25`,
`25-75`, `75-150`, `150-300`, `300-600`, `600-1200`, both signs where
applicable.

Step 4: teacher labeling.
Label the fresh positions with Stockfish:

- main labels: depth `16`
- premium subset: depth `18` on `2-4M` high-quality signed-bucket rows

Step 5: tensor packing.
Pack the labeled JSONL rows into the tensor format consumed by `train.py`.

Step 6: training matrix.
Train a small matrix:
...
Reject with held-out validation first. Include validation from the fresh
distribution and old d16/self-play/Lichess sets. Do not use binpack or hardcase
data as a main training source.

Step 8: replay gates.
Run replay gates on known Enyo misses/blunders/time losses. Current `replay`
