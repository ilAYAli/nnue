...
SELFPLAY_RUNNER="${SELFPLAY_RUNNER:-$HOME/tmp/enyo-own-net-pipeline/tools/selfplay/run_selfplay.sh}"
PGN_TO_JSONL="${PGN_TO_JSONL:-$HOME/tmp/enyo-own-net-pipeline/tools/selfplay/pgn_to_jsonl.py}"
ENGINE="${ENGINE:-$HOME/code/cpp/chess/assets/engines/reference}"
NNUE2_FILE="${NNUE2_FILE:-$ENGINE_REPO/nnue/berserk-d43206fe90e4.nn}"
...
for required in "$PY" "$SELFPLAY_RUNNER" "$PGN_TO_JSONL" "$ENGINE" "$NNUE2_FILE" \
