...
run_screen_sprt_for_best() {
  if [ ! -s "$RUN/passed_candidates.tsv" ]; then
    notify "Enyo NNUE binpack5m sweep: no candidate passed replay gate"
    exit 0
  fi
...
PY
  then
    notify "Enyo NNUE binpack5m sweep: best replay-clean candidate static_score=$score; no SPRT"
    exit 0
  fi
