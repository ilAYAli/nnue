...
class EnyoNNUE(nn_pt.Module):
    def __init__(self, init: str = "kaiming"):
        ...
        self.l2 = nn_pt.Linear(nn2.N_L2_HEAD, nn2.N_L3)
        ...
        if init == "kaiming":
            ...
            nn_pt.init.normal_(self.l2.weight, std=math.sqrt(2.0 / nn2.N_L2_HEAD))
    ...
    def raw_forward(self, w_feats: torch.Tensor, b_feats: torch.Tensor,
        ...
        phase_head = (phase_scale - 1.0).unsqueeze(-1)
        x2 = torch.relu(self.l2(torch.cat([x1, phase_head], dim=-1)))
