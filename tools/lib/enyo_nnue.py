...
NETWORK_V5_HEADER_MAGIC = b"ENYONN5\0"
...
def network_size(
    ...
    return (
        ...
        + (head_count * (N_L2 * N_L3 + N_L3) * np.dtype(np.float32).itemsize
        ...
        + (output_buckets * N_L2 * np.dtype(np.float32).itemsize
           if l2_output_skip else 0)
        + ((features * output_buckets + output_buckets) * np.dtype(np.float32).itemsize
           if psqt_residual else 0)
    )
...
class Net:
    ...
    psqt_weights: np.ndarray | None = None
    psqt_biases: np.ndarray | None = None
    l2_output_skip_weights: np.ndarray | None = None
...
def load_net(path: str | Path) -> Net:
    ...
    if data.startswith((NETWORK_V2_HEADER_MAGIC, NETWORK_V3_HEADER_MAGIC,
        ...
        l2_output_skip = bool(flags & NETWORK_FLAG_L2_OUTPUT_SKIP)
        if full_heads != (format_version in (3, 6, 11)):
            raise ValueError(f"{path}: v3/v6/v11 and full-head flag must be used together")
        if full_heads and output_buckets <= 1:
            raise ValueError(f"{path}: unsupported full-head architecture")
        if format_version == 3 and threat_features:
            raise ValueError(f"{path}: unsupported full-head architecture")
        ...
        if format_version == 11 and not (
            full_heads and not threat_features and not pawn_pairs and mixed_activation
            and l2_output_skip and output_buckets == 8 and output_head_features == 0
        ):
            raise ValueError(f"{path}: v11 requires full heads with mixed activation and L2-output skip")
    ...
    l2sb = None
    if mixed_activation:
        l2sw = take(np.float32, head_count * N_L2 * N_L3).reshape(
            head_count, N_L3, N_L2)
        l2sb = take(np.float32, head_count * N_L3).reshape(head_count, N_L3)
        if not full_heads:
            l2sw = l2sw[0]
            l2sb = l2sb[0]
    if not full_heads:
    ...
    l2osw = take(np.float32, output_buckets * N_L2).reshape(
        output_buckets, N_L2) if l2_output_skip else None
    psqtw = take(np.float32, n_features * output_buckets).reshape(
        n_features, output_buckets) if psqt_residual else None
    psqtb = take(np.float32, output_buckets) if psqt_residual else None
    ...
    return Net(
        ...
        l2_output_skip=l2_output_skip,
        l2_squared_weights=l2sw,
        l2_squared_biases=l2sb,
        psqt_weights=psqtw,
        psqt_biases=psqtb,
        l2_output_skip_weights=l2osw)
...
def write_net(net: Net, path: str | Path) -> None:
    ...
    if net.mixed_activation:
        ...
        squared_shape = ((head_count, N_L3, N_L2) if net.full_heads
                         else (N_L3, N_L2))
        squared_bias_shape = ((head_count, N_L3) if net.full_heads else (N_L3,))
        if l2_squared_weights.shape != squared_shape:
            raise ValueError("mixed activation has incompatible l2_squared_weights shape")
        if l2_squared_biases.shape != squared_bias_shape:
            raise ValueError("mixed activation has incompatible l2_squared_biases shape")
        squared_payload = (
    ...
    threat_features = net.full_threats or net.slider_xray_threats
    ...
    psqt_payload = b""
    if net.psqt_residual:
        psqt_weights = np.asarray(net.psqt_weights, dtype=np.float32)
        psqt_biases = np.asarray(net.psqt_biases, dtype=np.float32)
        if psqt_weights.shape != (expected_features, net.output_buckets):
            raise ValueError("PSQT residual requires psqt_weights[features,output_buckets]")
        if psqt_biases.shape != (net.output_buckets,):
            raise ValueError("PSQT residual requires psqt_biases[output_buckets]")
        psqt_payload = (
            psqt_weights.tobytes(order="C") + psqt_biases.tobytes(order="C"))
    l2_output_skip_payload = b""
    if net.l2_output_skip:
        l2_output_skip_weights = np.asarray(
            net.l2_output_skip_weights, dtype=np.float32)
        if l2_output_skip_weights.shape != (net.output_buckets, N_L2):
            raise ValueError("L2-output skip requires l2_output_skip_weights[buckets,16]")
        l2_output_skip_payload = l2_output_skip_weights.tobytes(order="C")
    payload = b"".join((
    ...
    if net.format_version == 1:
        ...
        if net.format_version == 11 and not (
            net.full_heads and not threat_features and not net.pawn_pairs
            and net.mixed_activation and net.l2_output_skip
            and net.output_buckets == 8 and net.output_head_features == 0
        ):
        ...
        magic = (
            ...
            else NETWORK_V6_HEADER_MAGIC if net.full_heads and threat_features
        ...
        header = NETWORK_HEADER.pack(
            ...
            | (NETWORK_FLAG_L2_OUTPUT_SKIP if net.l2_output_skip else 0),
            len(payload),
            0,
            0,
        )
    ...
    if net.format_version in (2, 3, 4, 5, 6, 8, 9, 10, 11):
