...
usage() {
  cat <<'EOF'
usage: tools/nnue/train_new_net_pwa.sh [options]
...
Examples:
  ...
  tools/nnue/train_new_net_pwa.sh --selfplay-rows 12000000 --lichess-rows 3000000
  tools/nnue/train_new_net_pwa.sh --foreground
EOF
}
...
active_pattern='run_selflichess_mix_pwa\.sh|tools/nnue/train\.py|tools/nnue/train_pairwise\.py|/sprt |fastchess'
...
cmd=(
  ...
  "SPRT_HASH=$sprt_hash"
  "SPRT_ELO1=$sprt_elo1"
  tools/nnue/run_selflichess_mix_pwa.sh
)
