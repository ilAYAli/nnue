...
INIT="$ENGINE_REPO/nnue/berserk-d43206fe90e4.nn"
...
for required in \
  "$PY" "$NNUE_REPO" "$ENGINE_REPO" "$INIT" "$ENGINE" "$SPRT" \
  "$D16_JSON" "$BINPACK_JSON" \
...
run_sprt_for_best() {
  if [ ! -s "$RUN/passed_candidates.tsv" ]; then
    notify "Enyo NNUE source-mix 1M: no candidate passed replay infrastructure gate"
    exit 0
  fi
...
PY
  then
    notify \
      "Enyo NNUE source-mix 1M: best gate-clean candidate static_score=$score; no SPRT"
    exit 0
  fi
