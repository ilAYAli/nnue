...
MATRIX_MODE="${MATRIX_MODE:-first}"
OLD_SOURCE_JSON="${OLD_SOURCE_JSON:-$HOME/tmp/enyo_teacher/sf_d12_20m_20260510_115338/labeled.jsonl}"
...
for required in "$PY" "$TOOLS/train.py" "$TOOLS/eval_dataset.py" \
  "$TOOLS/mix_jsonl.py" "$TOOLS/pack_dataset.py" \
  "$TOOLS/dataset_novelty_report.py" "$SOURCE_JSON" \
...
train_one() {
  ...
  notify "Enyo NNUE fresh d16 train matrix: train done $tag"
}
...
eval_candidate() {
  ...
  notify "Enyo NNUE fresh d16 train matrix: eval done $tag"
}
...
make_mix() {
  ...
  if [ ! -s "$MIX_JSON" ]; then
    ...
    notify "Enyo NNUE fresh d16 train matrix: 85/15 mix ready rows=$rows"
  fi
  if [ ! -s "$MIX_PACKED/meta.json" ]; then
    ...
    notify "Enyo NNUE fresh d16 train matrix: packed 85/15 mix ready $MIX_PACKED"
  fi
}
...
if [ ! -s "$RUN/source_novelty.txt" ]; then
  notify "Enyo NNUE fresh d16 train matrix: source novelty report"
  novelty_args=("$TOOLS/dataset_novelty_report.py" --input "$SOURCE_JSON")
  if [ -s "$OLD_SOURCE_JSON" ]; then
    novelty_args+=(--compare "$OLD_SOURCE_JSON")
  fi
  "$PY" "${novelty_args[@]}" | tee "$RUN/source_novelty.txt"
  notify "Enyo NNUE fresh d16 train matrix: source novelty report ready $RUN/source_novelty.txt"
fi
...
if [ "$MATRIX_MODE" != "full" ]; then
  notify "Enyo NNUE fresh d16 train matrix first-pass ready run=$RUN tag=fresh_huber_cp800_lr7e7_e8"
  exit 0
fi
