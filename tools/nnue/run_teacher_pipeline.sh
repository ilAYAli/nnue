...
"$python" tools/nnue/sample_jsonl.py \
  --input "$source_jsonl" \
  --output "$sample" \
  --rows "$sample_rows" \
  --seed 20260510 \
...
for shard in $(seq 0 $((shards - 1))); do
  ...
  "$python" tools/nnue/label_with_uci.py \
    --input "$sample" \
    --output "$shard_out" \
    --engine "$engine" \
    --depth "$depth" \
...
"$python" tools/nnue/pack_dataset.py \
  --input "$labeled" \
  --out-dir "$packed" \
  --progress 25000 | tee "$out/pack.log"

log "baseline metrics"
"$python" tools/nnue/eval_dataset.py \
  --net "$init_net" \
  --data "$packed" \
  --skip "$val_skip" \
  --rows "$val_rows" \
