#!/usr/bin/env python3
"""Sweep scaled Enyo .nn deltas through the broad search-target gate."""
...
def read_summary(path: Path, label: str) -> list[str]:
    ...
    for split in ("compare:all", "compare:mate_like", "compare:non_mate"):
        values = sections.get(split, {})
        candidate_values = sections.get(split.removeprefix("compare:"), {})
        out.append("\t".join([
            label,
            split,
            ...
            values.get("median_nonzero_diff_cp", ""),
            values.get("worst_regression_cp", ""),
            values.get("best_gain_cp", ""),
        ]))
    return out
...
def main() -> int:
    ...
    parser.add_argument("--scales", required=True, type=parse_scales)
    ...
    scale_tool = Path(__file__).with_name("scale_net_delta.py")
    ...
    emit_event(
        run_dir,
        "phase_start",
        stage="delta_scale_sweep",
        status="running",
        hook_command=args.event_command,
        extra={"scales": [scale_label(scale) for scale in args.scales]},
    )

    rows = [
        "scale\tsplit\ttop1\ttop3\tcandidate_better\treference_better\t"
        "capped_sum_diff_cp\tmedian_nonzero_diff_cp\tworst_regression_cp\tbest_gain_cp"
    ]
    rc = 0
    for scale in args.scales:
        label = scale_label(scale)
        stage = f"scale_{label}"
        net = nets_dir / f"delta_scale_{label}.nn"
        emit_event(
            run_dir,
            "phase_start",
            stage=stage,
            status="running",
            hook_command=args.event_command,
            extra={"scale": scale, "net": str(net)},
        )

        scale_rc = run([
            sys.executable, str(scale_tool),
        ...
        if scale_rc != 0:
            rc = scale_rc
            emit_event(
                run_dir, "fail", stage=stage, status="failed", rc=scale_rc,
        ...
        emit_event(
            ...
            log=summary_path,
            hook_command=args.event_command,
            extra={"scale": scale, "summary": str(summary_path)},
        )
        if gate_rc != 0:
            rc = gate_rc
            break
