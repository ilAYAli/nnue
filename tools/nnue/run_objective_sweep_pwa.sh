...
RUN="${RUN:-$HOME/tmp/enyo_teacher/objective_sweep_$(date +%Y%m%d_%H%M%S)}"
...
MIXED_PACKED="${MIXED_PACKED:-$HOME/tmp/enyo_teacher/source_mix_1m_20260513_142124/mix_d16_500k_binpack_500k_all_huber_lr1e6_e8/packed}"
...
notify "Enyo NNUE objective sweep: start $RUN"

for required in \
  "$PY" "$NNUE_REPO" "$ENGINE_REPO" "$INIT" "$ENGINE" "$SPRT" "$BOOK" \
  "$MIXED_PACKED" "$D16_PACKED" "$SELFPLAY_PACKED" "$LICHESS_PACKED" \
  "$BINPACK_PACKED" "$BASE_SUMMARY" "$REPLAY"
do
  if [ ! -e "$required" ]; then
    notify "Enyo NNUE objective sweep: missing $required"
    exit 1
  fi
done
...
train_candidate() {
  ...
  if [ ! -s "$dir/model.nn" ]; then
    notify "Enyo NNUE objective sweep: train $tag"
    "$PY" "$TOOLS/train.py" \
      --data "$MIXED_PACKED" \
      --init-from-nn "$INIT" \
      --epochs 8 \
  ...
  notify "Enyo NNUE objective sweep: train done $tag"
}
...
eval_candidate() {
  ...
  local dir="$RUN/$tag"
  mkdir -p "$dir/eval"
  notify "Enyo NNUE objective sweep: eval $tag"
  eval_baseline
...
gate_candidate() {
  ...
  local gate="$dir/replay_gates"
  make_wrapper "$tag"
  notify "Enyo NNUE objective sweep: replay gate $tag"
  : > "$gate/replay.log"
  : > "$gate/summary.txt"
  local logs=(
...
PY
  ...
  notify "Enyo NNUE objective sweep: SPRT screen finished $tag rc=$sprt_rc ${final_status:-no final status}"
  exit "$sprt_rc"
}

: > "$RUN/passed_candidates.tsv"

train_candidate "mix1m_mpe25_lam075_srcw_lr1e6_e8" \
  --objective mpe25 \
  --select-metric loss \
  --wdl-lambda 0.75 \
  --source-wdl-lambda stockfish_binpack=1.0 \
  --source-loss-weight stockfish=1.25 \
  --source-loss-weight stockfish_binpack=0.75 \
  --target-clamp 1600
eval_candidate "mix1m_mpe25_lam075_srcw_lr1e6_e8"
gate_candidate "mix1m_mpe25_lam075_srcw_lr1e6_e8"

train_candidate "mix1m_mpe25_cp_only_lr1e6_e8" \
  --objective mpe25 \
  --select-metric loss \
  --wdl-lambda 1.0 \
  --target-clamp 1600
eval_candidate "mix1m_mpe25_cp_only_lr1e6_e8"
gate_candidate "mix1m_mpe25_cp_only_lr1e6_e8"

train_candidate "mix1m_huber_cp800_lr1e6_e8" \
  --objective huber \
  --huber-beta 200 \
  --select-metric mae \
  --target-clamp 800
eval_candidate "mix1m_huber_cp800_lr1e6_e8"
gate_candidate "mix1m_huber_cp800_lr1e6_e8"
